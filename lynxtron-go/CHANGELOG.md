...
- edb37d2: Upgrade the coordinated Lynxtron runtime, CEF, builder, development plugins, library headers and rebuild tool to 0.0.28. This includes the preload Node-API asynchronous callback context fix and the production-dependency AutoLink fix that keeps CEF registration and initialization on the same staged native instance.
...
- 239b841: Upgrade the bundled Lynxtron runtime and coordinated CEF, native headers and build tooling from 0.0.23 to 0.0.25. Rebuild native extensions against the new runtime, including upstream Windows CEF OSR and macOS surface lifetime fixes. Release a new Go compatibility lane rather than changing the runtime in an existing showcase channel.
...
- Updated dependencies [ca72af6]
  - @lynxtron-examples/cli@0.0.11
...
- e16289f: Fix packaged showcase, brand, and help resources, allow external app resources through the Lynx file loader, and make Blueprint icons render reliably with the latest Lynxtron runtime by using the font's native family and recreating text nodes after asynchronous font registration.
