...
- **Nothing that covers a native view may paint a background.** The native child draws above the renderer host's own layer — which is where the ROOT element's background lands — and below every descendant background, because Clay promotes those to their own sublayers. So one `background-color` anywhere on the path down to the view is an opaque sheet over it, and the region comes up blank while layout, mounting and content are all perfectly correct. Spend backgrounds on leaves that do not overlap the native view, and let the root supply the ground. This is the cost of hosting the native view inside the Lynx hierarchy (below), and it is easy to misread as a compositor bug — the trace says "Mounting in Lynx renderer host" with the right rect either way.
- **A `<cover-view>` overlay swallows input for the whole window while it is up.** Not just where it paints — everything outside it goes deaf, and `pointer-events: none` on the host does not reach the platform layer. Measured: with the gallery open, a synthetic tap on the commands bar's own Gallery toggle does nothing, while the identical tap works with no overlay. So every full-page overlay must carry its own exit INSIDE itself; a control in the shell behind it is unreachable, and a surface whose only way back is out there is a trap.
