...
// Imported by: entries.ts (rspeedy entry map), src/home (gallery UI),
// src/main/desktop (window options + launch validation).
//
// `status`:
...
//              'na' fiddles have no `dir`/entry and are shown disabled.
//
// `id` is the rspeedy entry name AND the bundle basename (kebab-case, unique).
// `dir` is the folder under src/fiddles/ (kept equal to `id`).
