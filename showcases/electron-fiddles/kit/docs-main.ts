import { shell, lynxBridge } from '@lynx-js/lynxtron';
 ...
 *
 * The `win` argument is retained for API compatibility but is not used — the
 * handler lives on the process-global `lynxBridge`, so a single registration
 * covers every window in the process.
 *
 ...
 * primitive.
 */
export function attachDocsLinks(_win: LynxWindow): void {
  if (attached) return;
  attached = true;
  lynxBridge.on('open-docs', (data) => {
