...
export function App() {
  ...
  const sharedIntervalRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const lastTopBarTapTimeRef = useRef<number>(0);
  const topBarTapCountRef = useRef<number>(0);
  const topBarTapTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  return (
    <view clip-radius="true" className="page-root">
      <view className="top-bar" bindtap={handleTopBarTap}>
        ...
        {SystemInfo?.platform === 'windows' && (
          <view className="window-controls-group">
