// Adapted from upstream browser-demo: fix tab/close handlers and desktop typings.
import { useState, useRef, useEffect, useCallback } from '@lynx-js/react';
import type { InputProps } from '@lynx-js/types';
import { PAGE_INFO_SCRIPT } from './page-info.mjs';
import './App.css';
...
export function App() {
  ...
  // Clay input uses setValue, not an HTML-style controlled `value` attribute.
  useEffect(() => {
    lynx.createSelectorQuery().select('#browser-address').invoke({
      method: 'setValue', params: { value: inputValue },
      fail: (error) => console.error('Cannot update browser address', error),
    }).exec();
  }, [inputValue]);
  ...
  useEffect(() => {
    ...
    sharedIntervalRef.current = setInterval(() => {
      ...
      const pageInfoCode = PAGE_INFO_SCRIPT;
      ...
      webviewElement
        ...
        .exec();
    }, 5000); // Refresh metadata for page-driven title/URL changes every 5 seconds.
  ...
  const handleInput: InputProps['bindconfirm'] = (event) => {
    'background only';
  ...
  const handleRemoveTab = (tabIdToRemove: string) => {
    ...
    if (selectedTabId === tabIdToRemove) {
      if (updatedTabs.length > 0) {
        ...
        setInputValue(updatedTabs[newSelectedIndex].actual_url || updatedTabs[newSelectedIndex].input_value || '');
      } else {
        ...
        NativeModules.bridge.call('closeWindow', {}, (res: any) => {
          console.log(res);
        });
        setSelectedTabId('');
      }
  ...
  const handleOpenWindow = (event: any) => {
    ...
    if (!u) return;
    const newTabId = `tab-${nextTabId.current++}`;
    const newTab = { id: newTabId, icon: 'lynxtron' as const, title: u, input_value: u };
    setTabs([...tabs, newTab]);
    setSelectedTabId(newTabId);
    setInputValue(u);
  };
            ...
            {tab.input_value && (
              <webview
                ...
                bindload={() => {
                  ...
                  const initJsCode = PAGE_INFO_SCRIPT;
