...
const DESKTOP_FALLBACK: Rect = { left: 588, top: 58, width: 336, height: 238 };
...
export function useDesktopDrag() {
  ...
  const handleDesktopLayout = (event: any) => {
    ...
    setDesktopRect((current) => {
      try {
        const base = current ?? DESKTOP_FALLBACK;
        return {
          left: layout.left ?? base.left,
          top: layout.top ?? base.top,
          width: layout.width ?? base.width,
          height: layout.height ?? base.height,
        };
      } catch {
        return current ?? DESKTOP_FALLBACK;
      }
    });
  };
